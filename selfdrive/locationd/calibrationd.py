...
if HARDWARE.get_device_type() == 'mici':
  # Fix A: widened mici pitch box to give road-geometry-bias headroom.
  # Original: [-0.143101, 0.22235988]  (~-8.2° to +12.7°)
  # Widened: [-0.21, 0.30]              (~-12.0° to +17.2°)
  # Leaves ~9° extra margin total, split across both edges.
  # Spread check (MAX_ALLOWED_PITCH_SPREAD) still catches real mount shifts.
...
def is_calibration_valid(rpy: np.ndarray, bypass: bool = False) -> bool:
  # Fix C: when bypass is enabled (via CalibrationBoxCheckDisabled param),
  # the box check is short-circuited to True. The spread check downstream
  # in update_status() still runs, so genuine mount shifts still trigger
...
class Calibrator:
  def __init__(self, param_put: bool = False):
    ...
    # Fix C: live-readable bypass flag for the box check.
    # Refreshed in update_status() so the user can toggle it during a drive.
    ...
    # Fix C: one-shot user-requested reset → show as "Recalibrating" instead
    # of "Calibrating" (which is reserved for first-time setup). The flag is
    # consumed here so it doesn't persist beyond the next restart.
    if param_put:
      try:
  ...
  def update_status(self) -> None:
    # Fix C: poll bypass param at ~1 Hz so the user can toggle it live.
